...
const SEPARATION = 20,
  AMOUNTX = 64 + 2 * 4,
  AMOUNTY = 128 + 2 * 4;
let wsPointData = new Array(2048).fill(1);
...
class Anta extends React.Component {
  message(e) {
    ...
    if (jsonObject.data != null) {
      ...
      console.log(wsPointData1[wsPointData1.length - 1])
      

      if (wsPointData1[wsPointData1.length - 1] == 0) {
        firstData = [...wsPointData1];
        firstData.pop();
        // 右边线序
        // console.log(firstData.length , 'firstData')
      }else if (wsPointData1[wsPointData1.length - 1] == 1) {
        lastData = [...wsPointData1];
        lastData.pop();
        // console.log(lastData.length , 'lastData')
        // 添加
      ...
      if (this.breatheData.current) {
        this.breatheData.current.innerText = `${parseInt(
          jsonObject.breath_rate
        )}/次`;
        this.moveData.current.innerText = `${parseInt(
          jsonObject.bodymove_data
        )}/次`;
